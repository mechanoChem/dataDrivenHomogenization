...
if __name__ == '__main__':
    ...
    plt.legend()
    # plt.axis('equal')
    plt.savefig('m-dns-free-energy-learning-cnn.pdf', bbox_inches='tight', format='pdf')
    plt.show()
    ...
    plt.xlim([xmin, xmax])
    plt.ylim([xmin, xmax])
    plt.xlabel('$\psi_{\mathrm{mech,DNS}}$')
    plt.ylabel('$\psi_{\mathrm{mech,CNN}}$')
    plt.savefig('m-dns-free-energy-test-cnn.pdf', bbox_inches='tight', format='pdf')
    plt.show()
    ...
    plt.plot(ind0, all_prediction['label_dns'][0], 'k', label='DNS')
    plt.plot(ind0, all_prediction['label_nn'][0], 'r', label='DNN')
    for i in range(1, len(all_prediction['label_nn'])):
        plt.plot(ind0, all_prediction['label_dns'][i], 'k')
        plt.plot(ind0, all_prediction['label_nn'][i], 'r')

    plt.xlabel('frame number')
    plt.ylabel('$\psi_{\mathrm{mech}}$')
    plt.xlim([0, 900])
    # plt.axis('equal')
    plt.legend()
    plt.savefig('m-dns-free-energy-predict-all-cnn.pdf', bbox_inches='tight', format='pdf')
    plt.show()
