...
class HyperParametersCNN:
  ...
  """
    ...
    def __init__(self,
        ...
        print('... start parameter cnn preparation ')
        ...
        print('... done with parameter cnn preparation ')
    ...
    def prepare_parameter_search(self):
        ...
        if len(self.all_hyperparameter_info["all_model_structure"]) < self.best_model_number:
            print("...ERR...: Total CNN structure = ", len(self.all_hyperparameter_info["all_model_structure"]),
                  " is smaller than the best model number!!!", self.best_model_number)
            print("Please modify the hyperparameter search setting or delete previously saved file: ",
                  self.hyperparameter_filename)
            print(self.all_hyperparameter_info)
            exit(0)
    ...
    def update_model_info(self, index0, history):
        ...
        # print('type name: ', type(train_loss).__name__)
        ...
        self.all_hyperparameter_info['model_short_summary'][index0][1] = statistics.mean(
            self.all_hyperparameter_info['model_loss_summary'][index0]['loss'])
        self.all_hyperparameter_info['model_short_summary'][index0][2] = statistics.mean(
            self.all_hyperparameter_info['model_loss_summary'][index0]['val_loss'])
        print('mean(loss):', self.all_hyperparameter_info['model_short_summary'][index0][1], 'loss:',
              self.all_hyperparameter_info['model_loss_summary'][index0]['loss'])
    ...
    def generate_all_CNNs(self):
        ...
        # [1, 20]
        hidden_layer_num = ml_misc.getlist_int(self.config['HYPERPARAMETERS']['HiddenLayerNumber'])
        # print(hidden_layer_num)
        # [2, 512, 2]
        filter_num = ml_misc.getlist_int(self.config['HYPERPARAMETERS']['NodesList'])
        # print(filter_num)
        ...
        _tmp_parameters[1] = list(range(filter_num[0], filter_num[1],
                                        filter_num[2]))    # reverse the NodeList for decreasing filters, not common
        _tmp_studies = []

        if len(_tmp_parameters[1]) >= 18:
            print("... WARNING .. : combination n=", len(_tmp_parameters[1]), " choose r=", _tmp_parameters[0][-1],
                  " could be huge!!!")
    ...
    def do_the_sampling(self):
        ...
        index_list = np.linspace(
            self.all_hyperparameter_info["sampling_range"][0],
            self.all_hyperparameter_info["sampling_range"][1],
            self.uniform_sample_number + 2,
            dtype=int)
    ...
    def get_next_model(self):
        ...
        if (self.all_hyperparameter_info["current_iteration"] == self.iteration_time
            ...
            exit(0)
              ...
              and self.all_hyperparameter_info["current_index"] == len(self.all_hyperparameter_info["index_list"])):
            print("come to next iteration", self.all_hyperparameter_info["current_index"],
                  len(self.all_hyperparameter_info["index_list"]), self.all_hyperparameter_info["current_iteration"],
                  self.iteration_time)
        ...
        self.para_str = 'V' + str(this_CNN[2]) + '-' + 'model-' + 'H' + str(len(ml_misc.getlist_int(self.config['MODEL']['NodesList']))) \
    ...
    def plot_best_models(self, model_number_to_plot=5):
        ...
        for m0 in range(0, model_number_to_plot):
            ...
            print(self.all_hyperparameter_info["all_model_structure"][index0],
                  self.all_hyperparameter_info["model_short_summary"][index0])
    ...
    def plot_all_models(self):
        ...
        for s0 in short_summary:
            ...
            model_info.append('L' + str(self.all_hyperparameter_info["all_model_structure"][index0][0]) + 'N' +
                              str(self.all_hyperparameter_info["all_model_structure"][index0][1]))
