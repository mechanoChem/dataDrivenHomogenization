...
class MLKFold:
    ...
    """ at the end, 90% dataset will be split as 80%, 10% for train and validation """
    ...
    def get_next_fold(self, dataset, labels, derivative=[], fold_id=-1, final_data = False):
        ...
        print("In the fold: ", self.kfold_info["current_fold"], " with ", "train size: ",
              len(self.kfold_info["train_index_list"][self.kfold_info["current_fold"]]), "validation size: ",
              len(self.kfold_info["validation_index_list"][self.kfold_info["current_fold"]]))
    ...
    def init_kfold(self):
        ...
        print("init the kfold with n_splits = ", self.kfold_info["total_folds"], " size_of_dataset = ",
              self.kfold_info["size_of_data"], " current_fold = ", self.kfold_info["current_fold"])
    ...
    def sanity_check(self):
        """ perform data type check and TF version check for compatibility """
